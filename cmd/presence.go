...
// peerClient 复用的扇出客户端（在 main 里按 TLS 配置构造）
...
func globalUsers(peers []raft.Peer, secret, scheme string) []string {
	sets := [][]string{localUsers()}
	for _, peer := range peers {
		sets = append(sets, fetchPeerUsers(peerClient, peer, secret, scheme))
	}
...
func fetchPeerUsers(client *http.Client, peer raft.Peer, secret, scheme string) []string {
	req, err := http.NewRequest(http.MethodGet, scheme+"://"+peer.HTTPAddr+"/users?local=1", nil)
	if err != nil {
		return nil
	}
	if secret != "" {
